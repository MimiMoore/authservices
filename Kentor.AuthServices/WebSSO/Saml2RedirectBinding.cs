...
namespace Kentor.AuthServices.WebSso
{
    sealed class Saml2RedirectBinding : Saml2Binding
    {
            ...
            IOptions options)
        {
            ...
            if(!options.IdentityProviders.TryGetValue(new EntityId(issuer), out idp))
            {
                throw new InvalidSignatureException(
                    FormattableString.Invariant($"Cannot verify signature of message from unknown sender {issuer}."));
            }
        ...
        private static void CheckSignature(HttpRequestData request, IdentityProvider idp)
        {
            ...
            var signedString = FormattableString.Invariant(
                $"{msgParam}{relayStateParam}&SigAlg={rawQueryStringParams["SigAlg"]}");
            ...
            {
                throw new InvalidSignatureException(FormattableString.Invariant(
                    $"Message from {idp.EntityId.Id} failed signature verification"));
            }
        }
