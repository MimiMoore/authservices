    ...
    public class LogoutCommand : ICommand
    {
        ...
        private static void VerifyMessageIsSigned(UnbindResult unbindResult, IOptions options)
        {
            if (unbindResult.TrustLevel < TrustLevel.Signature)
            {
                ...
                {
                    throw new UnsuccessfulSamlOperationException(FormattableString.Invariant(
                        $"Received a {unbindResult.Data.LocalName} from {unbindResult.Data["Issuer", Saml2Namespaces.Saml2Name].InnerText} that cannot be processed because it is not signed."));
                }
            }
        }
        ...
        private static CommandResult HandleRequest(UnbindResult unbindResult, IOptions options)
        {
            ...
            if(options.SPOptions.SigningServiceCertificate == null)
            {
                throw new ConfigurationErrorsException(
                    FormattableString.Invariant($"Received a LogoutRequest from \"{request.Issuer.Id}\" but cannot reply because single logout responses must be signed and there is no signing certificate configured. Looks like the idp is configured for Single Logout despite AuthServices not exposing that functionality in the metadata."));
            }
        ...
        private static CommandResult HandleResponse(UnbindResult unbindResult, HttpRequestData request)
        {
            ...
            {
                throw new UnsuccessfulSamlOperationException(FormattableString.Invariant(
                    $"Idp returned status \"{status}\", indicating that the single logout failed. The local session has been successfully terminated."));
            }
