        ...
        public Saml2LogoutRequest CreateLogoutRequest()
        {
            if (spOptions.SigningServiceCertificate == null)
            {
                throw new InvalidOperationException(FormattableString.Invariant(
                    $"Tried to issue single logout request to {EntityId.Id}, but no signing certificate for the SP is configured and single logout requires signing. Add a certificate to the ISPOptions.ServiceCertificates collection, or to <serviceCertificates> element if you're using web.config."));
            }
